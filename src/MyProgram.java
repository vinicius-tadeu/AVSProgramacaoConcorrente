...
public class MyProgram {
    ...
    private static HashMap<String, JSONArray> GrupoCapitais(int length) throws InterruptedException, URISyntaxException, IOException {
        ...
        double total = Math.floor(jsonArray.length() / length);
        for (int i = 0; i < jsonArray.length(); i++) {
            String key = Double.toString(Math.round(i / total));
            JSONArray prevJsonArray = hashMap.get(key);
            Object item = jsonArray.get(i);
            if(prevJsonArray != null){
                prevJsonArray.put(item);
            }else{
                JSONArray newJsonArray = new JSONArray();
                newJsonArray.put(item);
                hashMap.put(key,newJsonArray);
            }
        }
